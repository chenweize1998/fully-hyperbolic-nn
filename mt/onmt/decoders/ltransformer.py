...
class LorentzTransformerDecoderLayer(nn.Module):
    ...
    def __init__(self,
        ...
        self.residual_1 = LorentzLinear(d_model, d_model, head_num=heads, merge=True, dropout=dropout, bias=False)
        self.residual_2 = LorentzLinear(d_model, d_model, head_num=heads, merge=True, dropout=dropout, bias=False)
...
class LorentzTransformerDecoder(DecoderBase):
    ...
    def __init__(self, num_layers, d_model, heads, d_ff, copy_attn,
        ...
        super(LorentzTransformerDecoder, self).__init__()
        ...
        self.manifold = Lorentz()
        self.dist_bias = nn.Parameter(torch.zeros(embeddings.embedding.data.shape[0]))
        self.decision = ManifoldParameter(deepcopy(embeddings.embedding.data), manifold=self.manifold)
    ...
    def from_opt(cls, opt, embeddings):
        ...
        return cls(
            ...
            if type(opt.attention_dropout) is list else opt.dropout,
            embeddings,
    ...
    def cal_dist(self, x):
        return (2 + 2 * self.manifold.cinner(x, self.decision)) + self.dist_bias
